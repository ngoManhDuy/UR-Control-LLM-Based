    ...
    async def transcribe_audio(self, audio_file):
        ...
        try:
            with open(audio_file, "rb") as audio:
                ...
                logging.info(f"Successfully transcribed audio: {response}")
                return response
    ...
    try:
        while True:
            if voice_mode:
                ...
                command = await voice_input.transcribe_audio(audio_file)
                
                if command:
                    print(f"\nTranscribed command: {command}")
                    if command.strip():  # Only process if command is not empty
                        await process_command(command, controller)
                    else:
                        print("Transcribed command was empty. Please try again.")
                else:
            ...
            if command and command.lower() == 'exit':
                break
            elif command and command.lower() == 'switch':
                voice_mode = not voice_mode
                print(f"\nSwitched to {'voice' if voice_mode else 'text'} input mode")
                continue
                
            if not voice_mode and command:
                await process_command(command, controller)
